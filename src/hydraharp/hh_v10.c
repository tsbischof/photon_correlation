...
int hh_v10_dispatch(FILE *in_stream, FILE *out_stream, pq_header_t *pq_header, 
	...
	if ( hh_header.MeasurementMode == HH_MODE_INTERACTIVE ) {
		result = hh_v10_interactive_stream(in_stream, out_stream, pq_header, 
				&hh_header, options);
...
int hh_v10_header_read(FILE *in_stream, hh_v10_header_t *hh_header,
	...
	result = fread(hh_header->InpChan, sizeof(hh_v10_input_channel_t),
...
int hh_v10_interactive_header_read(FILE *in_stream, 
	...
	interactive->Curve = (hh_v10_curve_t *)malloc(
			hh_header->NumberOfCurves*sizeof(hh_v10_curve_t));
	...
	result = fread(interactive->Curve, sizeof(hh_v10_curve_t),
...
int hh_v10_interactive_stream(FILE *in_stream, FILE *out_stream,
	...
	/* Read and print interactive data. */
		...
		hh_v10_interactive_data_print(out_stream, hh_header,
				&interactive, options);

		debug("Freeing interactive data.\n");
		hh_v10_interactive_data_free(hh_header, &interactive);
	}
		...
		if ( result != 1 && !feof(in_stream) ) {
			...
			if ( record.special ) {
				if ( record.channel == 63 ) {
					...
					warn("Sync event not yet implemented.\n");
				} else {
...
int hh_v10_tttr_stream(FILE *in_stream, FILE *out_stream,
	...
	if ( (result = hh_v10_tttr_header_read(in_stream, &tttr_header, options)) 
			!= PQ_SUCCESS ) {
	...
	if ( options->print_header ) {
		...
		if ( hh_header->MeasurementMode == HH_MODE_T2 ) {
			...
			result = hh_v10_t2_record_stream(in_stream, out_stream, 
					hh_header, &tttr_header, options);
		} else if ( hh_header->MeasurementMode == HH_MODE_T3 ) {
			debug("Found mode ht3.\n");
			result = hh_v10_t3_record_stream(in_stream, out_stream,
					hh_header, &tttr_header, options);
		} else {
	...
	hh_v10_tttr_header_free(&tttr_header);
