...
int th_v50_dispatch(FILE *in_stream, FILE *out_stream, pq_header_t *pq_header,
	...
	result = th_v50_header_read(in_stream, &th_header, options);
	if ( result ) {
		...
		if ( th_header.MeasurementMode == TH_MODE_INTERACTIVE ) {
			...
			result = th_v50_tttr_stream(in_stream, out_stream,
					pq_header, &th_header, options);
		result = PQ_MODE_ERROR;
		} else {
...
int th_v50_header_read(FILE *in_stream, th_v50_header_t *th_header,
	...
	result = fread(th_header, 
			sizeof(th_v50_header_t) - sizeof(th_v50_board_t *), 
			1, in_stream);
	if ( result != 1 ) {
	...
	th_header->Brd = (th_v50_board_t *)malloc(sizeof(th_v50_board_t)
	...
	result = fread(th_header->Brd,
			sizeof(th_v50_board_t), th_header->NumberOfBoards, in_stream);
...
int th_v50_interactive_read(FILE *in_stream,
		th_v50_header_t *th_header,
...
int th_v50_interactive_stream(FILE *in_stream, FILE *out_stream,
	...
	th_v50_interactive_free(&interactive, th_header);
	return(PQ_SUCCESS);
}
 ...
 *
 */
void th_v50_header_print(FILE *out_stream, 
		th_v50_header_t *th_header) {
	int i;
...
void th_v50_interactive_data_print(FILE *out_stream, 
		th_v50_header_t *th_header, 
		th_v50_interactive_t **interactive,
		options_t *options) {
	int i;
...
int th_v50_tttr_stream(FILE *in_stream, FILE *out_stream, 
	...
	if ( options->print_header ) {
		...
		print_resolution(out_stream, th_header->Brd[0].Resolution*1e3, options);
	} else {
...
void th_v50_tttr_header_print(FILE *out_stream, 
	...
	fprintf(out_stream, "SpecHeaderLength = %"PRId32"\n", 
			tttr_header->SpecHeaderLength);

	for ( i = 0; i < tttr_header->SpecHeaderLength; i++ ) {
		fprintf(out_stream, "SpecHeader[%d] = %"PRId32"\n",
