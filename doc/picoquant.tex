...
\newcommand{\integrationtime}{\ensuremath{\Lambda}}
...
\begin{equation}
\photons_{c}(t) = \abs{\braces{\photon\left|\begin{split}
                                          \photon\in\photons; \\
                                           \Channel(\photon)=c;\\
                                           \Time(\photon)=t
                                     \end{split}\right.}}
...
\angles{I(t)I(t+\tau)}\propto
         \abs{\braces{(\photon_{0},\photon_{1})
               \left|\begin{split}
                     ...
                     \end{split}\right.}}
\end{equation}
This is accurate up to a normalization constant, which is related to the units of \wholes. In a real-world experiment, the time resolution $\resolution$ will carry some units (e.g. picoseconds), such that the true time is some multiple $n\epsilon$ for $n\in\wholes$. Here we define $\resolution=1$, but later this value will change and it is important to keep track of it in the definition. As such, given the time resolution $\resolution$ indicating that the function \Time(\photon) really reports the number of photons in a time span $[t, t+\resolution)$, the normalized average correlation becomes
\begin{equation}
\angles{I(t)I(t+\tau)}=    
         \frac{1}{\resolution^{2}}
         \abs{\braces{(\photon_{0},\photon_{1})
...
\begin{equation}
\gn{2}(\tau) = \frac{\integrationtime^{2}}{\abs{\photons}^{2}\resolution^{2}}
                \abs{\braces{(\photon_{0}, \photon_{1}),
                              \left|\begin{split}
                              \photon_{0},\photon_{1}\in\photons \\
                              \Time(\photon_{1})-\Time(\photon_{0})=\tau
                              \end{split}\right.}}
\end{equation}
Extension of this result to higher dimensions and multiple channels proceeds much as before, resulting in the following two-channel cross-correlation:
\begin{equation}
\gn{2}(\tau) = \sum\limits_{(c_{0},c_{1})\in C^{2}}
                    {
                    \frac{\integrationtime^{2}}
                         {\abs{\photons_{c_{0}}}\abs{\photons_{c_{1}}}
                                \epsilon_{c_{0}}\epsilon_{c_{1}}}
                    \abs{\braces{(\photon_{0},\photon_{1})
                          \left|\begin{split}
...
\gn{n}&(\tau_{1},\ldots\tau_{n-1}) \\
    & = \sum\limits_{\vec{c}\in C^{n}}
                    ...
                    \left[
                    \left(
                    \prod_{j=0}^{n-1}{\frac{\integrationtime}
                                           {\abs{\photons_{c_{j}}}\resolution_{c_{j}}}}
                    \right)
