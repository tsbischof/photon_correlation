...
\Histogram(a_{1},\ldots a_{n-1}) = \abs{\braces{(\photon_{0}.\ldots\photon_{n-1})
                                              ...
                                              \end{aligned}\right.}}
\end{equation}
where the ranges $b_{a_{j}}$ have replaced the implicit ranges $[\tau_{j},\tau_{j}+\epsilon_{c_{j}})$, and the channel definitions have been set previously. More exactly, this is a function of a set of photons which returns a function mapping bin defintions to whole numbers, but this expression here should be sufficient to convey the general understanding of what must happen.
